# Mentions légales

Ce site est édité par _Gaëtan Poblon_ à titre non professionnel.

Les rédactions et les contributions sont également réalisées par _Gaëtan Poblon_, auteur.

Ce site est hébergé par OVH SAS 2 rue Kellermann - 59100 Roubaix - France. Contact : 1007.

Ces mentions légales sont définies conformément à l'[article 1-1-II de la loi n° 2004-575 du 21 juin 2004](https://www.legifrance.gouv.fr/loda/article_lc/LEGIARTI000049568614).

Les coordonnées personnelles de l'éditeur ont été transmises à l'hébergeur, qui est tenu au secret professionnel.

# Politique de confidentialité

Le site et l'éditeur respectent votre vie privée et s'engagent à protéger vos données personnelles.

D'une façon générale, vous pouvez visiter le site sans avoir à décliner votre identité et à fournir des informations personnelles vous concernant.

Fait office d'unique exception le formulaire de contact : il est nécessaire de fournir une adresse e-mail et une identité.
Les données collectées via ce formulaire sont utilisées uniquement par l'éditeur, pour répondre à votre demande et ne sont pas conservées au-delà de la durée nécessaire à cette fin.

Aucune utilisation de données personnelles n'est faite par le site ou l'éditeur en dehors de l'exception susmentionnée.

Aucune donnée utilisateur n'est partagée avec des tiers.

D'une manière générale, ce site ne fait usage d'aucun cookie de suivi ou de traçage, et ne collecte aucune donnée de navigation.

S'il en est, toute exception est indiquée de manière claire et transparente ci-dessous :

- _Aucune exception. Votre vie privée doit être respectée et protégée._

# Conditions générales d'utilisation

## Propriété intellectuelle

Le site et chacun des éléments et des contenus, y-compris mais sans limitation les marques, les logos, icônes, articles, qui le composent sont protégés au titre de la législation internationale de la propriété intellectuelle.
Les contenus figurant sur le site sont la propriété de l'éditeur ou d’autres entités.
Toute utilisation, reproduction ou représentation, par quelque procédé que ce soit, et sur quelque support que ce soit, de tout ou partie du site et/ou des éléments qui le composent n'est pas autorisée sans le consentement expresse de l'éditeur.

L'editeur se réserve le droit de poursuivre toute utilisation non autorisée du site et de ses éléments, y-compris par voie judiciaire.

L'éditeur fait usage de contenus tiers appartenant aux entités suivantes :
- DioxusLabs (OSS sous licence MIT et Apache-2), pour le développement du site ainsi qu'une icône.
- D'autres bibliothèques et technologies open source sous licence MIT ou Apache-2.

L'auteur cite également des technologies privées sous licence commerciale, ou des technologies open source sous licences variées.
Aucune forme de partenariat ou d'affiliation n'existe entre l'auteur et les entités propriétaires de ces technologies, et aucune forme de rémunération n'est versée à l'auteur par ces entités.

## Responsabilité de l'éditeur

Les informations et documents figurant sur ce site sont fournis gracieusement à titre informatif et collaboratif.
Bien que l'auteur s'efforce de fournir des informations fiables, vérifiables, sourcées, et à jour, il ne saurait être tenu responsable des erreurs, omissions ou d'une absence de mise à jour des informations mises à disposition.
L'utilisation de ces informations et contenus disponibles sur l'ensemble du site ne saurait en aucun cas engager la responsabilité de l'éditeur.
L'utilisateur est seul responsable de l'utilisation qu'il fait des informations et contenus disponibles sur le site, et de toute conséquence qui pourrait en découler.

L'éditeur, par le biais de son hébergeur, s'efforce de maintenir le site accessible 24/7.
L'éditeur n'est tenu à aucune obligation de disponibilité du site et ne saurait être tenu repsonsable de tout dommage résultatant d'une indisponibilité du site.

## Droit applicable

Les présentes mentions légales et conditions d'utilisation sont régies par le droit français. En cas de litige, les tribunaux du domicile de l'éditeur seront seuls compétents.


# Utilisation de l'Intelligence Artificielle

Aucune Intelligence Artificielle n'est utilisée pour la génération de ce code source.

Sans exception, les articles sont le fruit de la réflexion, de la recherche et de l'expérience personnelles de l'auteur, et non d'une Intelligence Artificielle.

Cependant, dans le strict respect de la plume et de l'intégrité intellectuelle de l'auteur, l'Intelligence Artificielle peut être utilisée dans les cadres suivants :
- La vérification des articles dans une perspective de correction et de cohérence du contenu,
- La traduction - en français, ou en anglais selon la rédaction initiale,
- La génération de fichiers SVG pour certaines icônes et certains éléments graphiques du site.

# Source

Le code source de ce site, son processus de déploiement ainsi que l'ensemble des articles qui le composent, est disponible publiquement à l'adresse suivante : https://github/gpoblon/folio.
